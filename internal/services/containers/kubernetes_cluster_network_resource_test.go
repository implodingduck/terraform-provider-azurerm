...
func TestAccKubernetesCluster_httpProxyConfig(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.httpProxyConfig(data, nil, ""),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
...
func TestAccKubernetesCluster_httpProxyConfigWithTrustedCa(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster", "test")
	r := KubernetesClusterResource{}
	trustedCa := ""
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.httpProxyConfig(data, nil, ""),
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("http_proxy_config.0.http_proxy").IsSet(),
				check.That(data.ResourceName).Key("http_proxy_config.0.https_proxy").IsSet(),
				check.That(data.ResourceName).Key("http_proxy_config.0.trusted_ca").HasValue(""),
			),
		},
		data.ImportStep(),
		{
			Config: r.httpProxyConfig(data, nil, trustedCa),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("http_proxy_config.0.http_proxy").IsSet(),
				check.That(data.ResourceName).Key("http_proxy_config.0.https_proxy").IsSet(),
				check.That(data.ResourceName).Key("http_proxy_config.0.trusted_ca").HasValue(trustedCa),
			),
		},
		data.ImportStep(),
	})
...
func TestAccKubernetesCluster_httpProxyConfigWithNoProxy(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster", "test")
	r := KubernetesClusterResource{}
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.httpProxyConfig(data, []string{"azurerm_subnet.test.address_prefixes.0, \"bing.com\""}, ""),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("http_proxy_config.0.http_proxy").IsSet(),
				check.That(data.ResourceName).Key("http_proxy_config.0.https_proxy").IsSet(),
...
func (KubernetesClusterResource) httpProxyConfig(data acceptance.TestData, noProxy []string, trustedCa string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%d"
  location = "%s"
...
`, data.RandomInteger, data.Locations.Primary, data.RandomInteger, data.RandomInteger, data.RandomInteger, data.RandomInteger, data.RandomInteger, data.RandomInteger, currentKubernetesVersion, data.RandomInteger, noProxy, trustedCa)
}
